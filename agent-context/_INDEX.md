...
Alphabetical list of all agent context files. Read only what you need.

| File | Lines (approx) | Description |
|------|----------------|-------------|
| `back/architecture.md` | ~50 | Vertical-slice layout, Program.cs startup flow |
| `back/infrastructure.md` | ~40 | Database, web middleware, extension methods |
| `client/architecture.md` | ~25 | Vite proxy, src layout (stub) |
| `conventions.md` | ~45 | Naming, DI, API routes, git habits |
| `devops/local-setup.md` | ~45 | Docker Postgres, env vars, node-pg-migrate |
...
**Not for agents:** `GUIDE.md` — human maintenance guide. Do not read unless attached.

**Navigation:** `_NAVIGATION.md` — problem → file map.
